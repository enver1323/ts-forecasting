...
from domain.bilinear.modules.model import BiLinear
...
class BiLinearTrainer(BaseTorchTrainer):
    def __init__(self, config: BiLinearConfig, device: torch.device):
        self.config: BiLinearConfig

        super(BiLinearTrainer, self).__init__(config, device)

    @property
    def model_type(self) -> Type[BiLinear]:
        return BiLinear

    def get_experiment_key(self, config: BiLinearConfig):
        model_name = str(self.model)
        model_name = model_name[:model_name.find('(')]
        seq_len = config.data.dataset.size.seq_len
        pred_len = config.data.dataset.size.pred_len
