...
class DishTS(nn.Module):
    def __init__(self, init_type: str, n_series: int, seq_len: int, activate: bool = True):
        ...
        if init == 'standard':
            ...
            self.reduce_mlayer = nn.Parameter(torch.ones(
                n_series, lookback, 2)/lookback+torch.rand(n_series, lookback, 2)/lookback)
        self.gamma, self.beta = nn.Parameter(torch.ones(
            n_series)), nn.Parameter(torch.zeros(n_series))
    ...
    def forward(self, batch_x, mode='forward', dec_inp=None):
        if mode == 'forward':
            ...
            dec_inp = None if dec_inp is None else self.forward_process(
                dec_inp)
    ...
    def preget(self, batch_x):
        ...
        self.xil = torch.sum(torch.pow(batch_x - self.phil, 2),
                             axis=1, keepdim=True) / (batch_x.shape[1]-1)
        self.xih = torch.sum(torch.pow(batch_x - self.phih, 2),
                             axis=1, keepdim=True) / (batch_x.shape[1]-1)
