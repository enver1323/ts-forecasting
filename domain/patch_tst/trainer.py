...
from domain.patch_tst.modules.model import PatchTST
from domain.patch_tst.config import PatchTSTConfig
...
class PatchTSTTrainer(BaseTorchTrainer):
    def __init__(self, config: PatchTSTConfig, device: torch.device):
        self.config: PatchTSTConfig

        super(PatchTSTTrainer, self).__init__(config, device)

    @property
    def model_type(self) -> Type[PatchTST]:
        return PatchTST

    def get_experiment_key(self, config: PatchTSTConfig):
