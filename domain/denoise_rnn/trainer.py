...
from domain.denoise_rnn.modules.model import DenoiseRNN
...
class DenoiseRNNTrainer(BaseTorchTrainer):
    def __init__(self, config: DenoiseRNNConfig, device: torch.device):
        self.config: DenoiseRNNConfig

        super(DenoiseRNNTrainer, self).__init__(config, device)

    @property
    def model_type(self) -> Type[DenoiseRNN]:
        return DenoiseRNN

    def get_experiment_key(self, config: DenoiseRNNConfig):
