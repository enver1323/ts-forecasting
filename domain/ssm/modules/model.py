...
from domain.ssm.modules.layers import SSMBlock
...
class SSM(eqx.Module):
    ...
    def __init__(self, config: SSMConfig.ModelConfig, *, key: KeyArray):
        ...
        emb_k, step_lin_k, blocks_k, combine_lin_k, pred_lin_k, norm_k = jrandom.split(
            key, 6
        )
        ...
        self.norm = jax.vmap(lambda k: DishTS(self.n_patches, config.d_model, key=k))(norm_ks)
    ...
    def process_patches(
        ...
        dynamic_layer_norm, static_layer_norm = eqx.partition(
            self.layer_norm, eqx.is_array
        )
        dynamic_norm, static_norm = eqx.partition(
            self.norm, eqx.is_array
        )

        def layer_step(_carry, _dynamic):
            _hidden, _ssm_state = _carry
            _dynamic_component, _dynamic_layer_norm, _dynamic_norm = _dynamic
            component = eqx.combine(_dynamic_component, static_components)
            layer_norm = eqx.combine(_dynamic_layer_norm, static_layer_norm)
            norm = eqx.combine(_dynamic_norm, static_norm)

            _hidden, _norm_state = norm(_hidden)
            _upd_hidden, _upd_ssm_state = jax.vmap(component)(
                _hidden, _ssm_state
            )
            _hidden = self.norm.denorm(_hidden, _norm_state)
        ...
        (hidden, ssm_state), _ = jax.lax.scan(
            layer_step, step_init, (dynamic_components, dynamic_layer_norm, dynamic_norm))
    ...
    def predict(self, x: Float[Array, "seq_len n_channels"]) -> Float[Array, "pred_len n_channels"]:
        ...
        preds = jax.vmap(self.pred_lin)(res_hidden)
        preds = preds.transpose(-1, -2)
